...
# 事前学習済みモデル
# 元となる、DS_B.tsv（akinobuさん作）を4エポックで学習させたモデルをダウンロード
PRETRAINED_MODEL_NAME = "./content_pre/content/model"  
if not os.path.exists(PRETRAINED_MODEL_NAME):
    gdd.download_file_from_google_drive(file_id='1qZmBK0wHO3OZblH8nabuWrrPXU6JInDc',
                                    dest_path='./content_pre/mT5-211221.zip',
                                    unzip=True)
...
if __name__ == '__main__':
    ...
    slack.notify(text='はじまったよん') # 指定したチャンネルに送信
    ...
    slack.notify(text='終わったよん') # 指定したチャンネルに送信